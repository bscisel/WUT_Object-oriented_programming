...
void MainWindow::display_question()
{
    Question *current_question = active_session->get_randomly_selected_questions()[
        active_session->get_current_question_index()
    ];
    ui->questionTextLabel->setText(QString::fromStdString(current_question->get_text()));
    const std::vector<Answer> &answers = current_question->get_answers();
    ui->answer1Button->setText(QString::fromStdString(answers[0].get_text()));
    ui->answer2Button->setText(QString::fromStdString(answers[1].get_text()));
    ui->answer3Button->setText(QString::fromStdString(answers[2].get_text()));
    ui->answer4Button->setText(QString::fromStdString(answers[3].get_text()));
...
void MainWindow::next_question() {
    unsigned short int current_question_index = active_session->get_current_question_index();
    Question *current_question = active_session->get_randomly_selected_questions()[current_question_index];
    std::vector<Answer> answers = current_question->get_answers();
    std::vector<Saved_answer> user_answers;

    if (ui->answer1Button->isChecked()) {
        user_answers.push_back(Saved_answer(answers[0]));
    }
    if (ui->answer2Button->isChecked()) {
        user_answers.push_back(Saved_answer(answers[1]));
    }
    if (ui->answer3Button->isChecked()) {
        user_answers.push_back(Saved_answer(answers[2]));
    }
    if (ui->answer4Button->isChecked()) {
        user_answers.push_back(Saved_answer(answers[3]));
    }
